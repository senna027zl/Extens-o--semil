// ============================================================
// Hannacore v2.0 — API real do SillyTavern
// ============================================================
...
function addMemory(mem, store = STORE_MEMORIES) {
return new Promise((resolve, reject) => {
const tx = db.transaction(store, "readwrite");
const req = tx.objectStore(store).add(mem);
...
Revelações sobre passado/corpo/hábitos
Padrões recorrentes
    ...
    const res = await fetch(apiEndpoint, {
        ...
        body: JSON.stringify({
            messages: [
                ...
                { role: "user", content: messages.slice(-15).map(m =>${m.name || m.role}: ${m.mes || m.content}`).join("\n") }
],
...
} catch (e) { console.warn("[Hannacore] Erro ao gerar resumo:", e); return null; }
}
...
// ========== INTERCEPTOR VIA EVENTO REAL DO ST ==========
async function onGenerateBeforeCompletion(data) {
if (!data?.prompt) return;
const chat = (typeof getContext === 'function' ? getContext()?.chat : null) || [];
...
if (config.autoDetect && messageBuffer.length >= 5) {
    ...
    if (spike) {
        ...
        if (summary) {
            ...
            await addMemory(mem).catch(() => {});
            if (config.semanticEnabled && config.githubToken && config.gistId) {
                const all = await getAllMemories().catch(() => []);
                await syncToGist({ timestamp: Date.now(), memories: all }).catch(() => {});
            }
            messageBuffer = [];
...
const memories = await getAllMemories().catch(() => []);
...
if (skillsText || memoryText) {
    data.prompt.system_prompt = `${data.prompt.system_prompt || <q>""</q>}\n\n---\n${skillsText ? skillsText + <q>"\n\n"</q> : <q>""</q>}${memoryText ? <q>"[Memórias]\n"</q> + memoryText : <q>""</q>}`.trim();
}
}
...
command: "hc-status",
callback: async () => {
const mems = await getAllMemories().catch(() => []);
const skills = extension_settings[EXT_NAME]?.skills || {};
const active = Object.entries(skills).filter(([_, s]) => s.active).map(([n]) => n);
toastr.info(📊 Hannacore | Memórias: ${mems.length} | Skills ativas: ${active.join(', ') || 'nenhuma'} | Semântica: ${config.semanticEnabled ? 'ligada' : 'desligada'});
},
helpString: "Mostra status do Hannacore"
});
SlashCommandParser.addCommandObject({
command: "hc-limpar",
callback: async () => {
await clearAllMemories().catch(() => {});
toastr.info("🗑 Todas as memórias locais foram apagadas.");
},
helpString: "Limpa todas as memórias locais"
});
SlashCommandParser.addCommandObject({
command: "hc-config",
...
if (parts.length === 0) {
toastr.info(⚙️ GitHub: ${config.githubToken ? '✔' : '✘'} | Gist: ${config.gistId ? '✔' : '✘'} | DeepSeek: ${config.deepseekApiKey ? '✔' : '✘'});
return;
}
const key = parts[0].toLowerCase();
const val = parts.slice(1).join(" ");
if (key === "githubtoken" && val) { config.githubToken = val; saveConfig(); toastr.success("✅ GitHub Token salvo"); }
else if (key === "gistid" && val) { config.gistId = val; saveConfig(); toastr.success("✅ Gist ID salvo"); }
else if (key === "deepseekapikey" && val) { config.deepseekApiKey = val; saveConfig(); toastr.success("✅ DeepSeek API Key salvo"); }
else if (key === "semanabled" && (val === "on" || val === "off")) { config.semanticEnabled = val === "on"; saveConfig(); toastr.success(Semântica ${config.semanticEnabled ? 'ligada' : 'desligada'}); }
else { toastr.warning("Uso: /hc-config githubtoken|gistid|deepseekapikey|semanabled "); }

},

helpString: "Configura Hannacore"
...
toastr.info("📦 Snapshot exportado");

},

helpString: "Exporta snapshot completo"
...
fab.title = "Hannacore";
fab.onclick = () => {
const p = document.getElementById("hc-config-panel");
...
panel.innerHTML = `
    ...
    <input type="password" id="hc-gh-token" value="${config.githubToken || ''}">
    <label>Gist ID</label>
    <input type="text" id="hc-gist-id" value="${config.gistId || ''}">
    <label>DeepSeek API Key</label>
    <input type="password" id="hc-ds-key" value="${config.deepseekApiKey || ''}">
    <div class="hc-btn-row">
      <button class="hc-btn-save" id="hc-save-btn">Salvar</button>
      <button class="hc-btn-toggle" id="hc-toggle-sem">Semântica: ${config.semanticEnabled ? 'ON' : 'OFF'}</button>
      <button class="hc-btn-close" id="hc-close-btn">Fechar</button>
    </div>
`;
...
document.getElementById("hc-toggle-sem").onclick = () => {
    ...
    saveConfig();
    const btn = document.getElementById("hc-toggle-sem");
    btn.textContent = `Semântica: ${config.semanticEnabled ? 'ON' : 'OFF'}`;
    toastr.info(`Semântica ${config.semanticEnabled ? 'ativada' : 'desativada'}`);
};
document.getElementById(<q>"hc-close-btn"</q>).onclick = () => {
    document.getElementById(<q>"hc-config-panel"</q>).style.display = <q>"none"</q>;
};
}
...
loadSkills();
openDB().then(() => {
console.log("[Hannacore] DB aberto");
}).catch(e => {
console.warn("[Hannacore] Erro ao abrir DB:", e);
});

if (typeof eventSource !== "undefined") {
    eventSource.on(event_types.GENERATE_BEFORE_COMPLETION, onGenerateBeforeCompletion);
    console.log("[Hannacore] Interceptor registrado via evento");
} else {
    console.warn("[Hannacore] eventSource não disponível");
}

registerSlashCommands();
createUI();
console.log("[Hannacore] v2.0 inicializado");
}
